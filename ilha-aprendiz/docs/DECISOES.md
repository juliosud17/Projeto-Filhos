...
**Contexto:** mesmo depois do `GRACE_MS` maior (entrada anterior), o Júlio continuou ouvindo o TTS atravessar/cortar a voz da Lia e das sílabas no celular. Pediu explicitamente: "tira isso dai logo, ta proibido tts por enquanto no módulo de monte as sílabas". Não é mais "dar mais tempo pro áudio real vencer a corrida" — é desligar o TTS de vez nesse módulo.

**Decisão de design:** como `AudioManager` (o único lugar do app que tinha lógica de fallback pra TTS) é usado exclusivamente pelo "Monte a Sílaba" hoje (confirmado por busca no código -- nenhuma outra atividade usa `AudioManager.playVoice`/`queueVoice`, todas as outras chamam `speak()` direto e continuam com TTS normal), dava pra desligar globalmente dentro do `AudioManager` sem afetar nada mais. Optei por um interruptor explícito (`AudioManager.setTtsAllowed(false)`), chamado 1x no topo de `renderSilabas()` (cobre os 5 níveis, incluindo o nível 5 que usa `renderDigitePalavra` via early return) -- em vez de simplesmente apagar o código de TTS, porque é reversível e documentado como decisão "por enquanto", igual o Júlio pediu, não uma remoção definitiva de arquitetura.

**O que mudou:**
- `app/js/audio-manager.js`: `AudioManager.setTtsAllowed(v)` novo -- quando `false`, `startTts()` nunca chama `speak()`, mesmo se o áudio real falhar de verdade (arquivo ausente/quebrado). Como o banco de mídia está 100% completo, isso não deixa nada mudo no caso normal -- só quando um arquivo realmente falhar é que a criança fica em silêncio nesse trecho específico, em vez de ouvir TTS.
- `app/js/activities-portugues.js`: `renderSilabas()` chama `AudioManager.setTtsAllowed(false)` logo no início.
- Efeito colateral bom: o delay artificial que existia só pra "dar tempo do TTS terminar de falar" (`Math.min(4000, 600+len*55)`) não faz mais sentido com TTS desligado -- virou um delay fixo curto (200ms) nesse caso, então a fila de voz resolve mais rápido quando cai no fallback, não mais devagar.

**Testes:** `qa_test_piloto_vaca.js` precisou de uma reescrita grande (seções 3-7) -- o arquivo inteiro foi construído em 2026-08-17 simulando "nenhum mp3 existe ainda" e observando o comportamento através do CONTEÚDO falado por TTS (`spokenLog`). Essa premissa está dupla e definitivamente ultrapassada: (1) a mídia está 100% completa desde 2026-08-19, (2) TTS agora está proibido nesse módulo. Reescrevi o stub de `Audio` pra simular SUCESSO por padrão (dispara `playing`/`ended`, reflete a realidade atual) e registrar as URLs tocadas em ordem (`audioLog`) -- as checagens de sequência/conteúdo (Lia → VA → CA → VACA, dica revela só a 1ª sílaba, etc.) passaram a verificar `audioLog` em vez de `spokenLog`. A seção 3 virou uma checagem dedicada e mais forte do que a anterior: simula uma falha REAL do áudio (stub trocado só ali) e confirma que o TTS continua NÃO tocando -- era exatamente esse comportamento que não existia até este pedido do Júlio. `qa_test_speak_coverage.js` ganhou uma exceção documentada pra `silabas` (a checagem genérica "toda atividade fala via TTS" não se aplica mais a esse módulo, de propósito).

Suíte completa: `qa_test_piloto_vaca.js` 838/838 (836→837 com a seção 3 reescrita, 838 com o ajuste fino do delay), `qa_test_speak_coverage.js` 26/26, sem falha nova em nenhum outro arquivo. Achado à parte, confirmado NÃO relacionado a esta mudança (reproduz idêntico com a mudança revertida): `qa_test_typing.js` tem uma flakiness pré-existente e rara (~1 em 10 rodadas) num loop de 15 rodadas do nível 5, provavelmente ligada à seleção aleatória de palavra batendo em algum caso de borda na comparação da resposta digitada -- não investigado a fundo agora, fica registrado pra quem for olhar depois.

---

## 2026-08-21 — Fase 2: Vite via `publicDir` passthrough (não bundling do app real)

**Contexto:** Fase 2 pedia oficialmente rodar o Ilha Aprendiz por `npm run dev`/`build`/`preview` (Vite), com objetivo único de trocar `file://` por HTTP, princípio central "Vite é infraestrutura, não autorização pra reescrever a arquitetura" e proibição explícita de converter os 24 scripts pra ES Modules ou processá-los. Isso exige uma decisão real: como fazer o Vite "encontrar" `app/ilha_aprendiz.html` sem processá-lo.

**Decisão:** `vite.config.js` usa `publicDir: 'app'` — todo o conteúdo de `app/` (HTML real, `css/`, `data/`, `js/`, `assets/`) é copiado (build) ou servido (dev) por Vite como passthrough puro, sem nenhuma transformação, exatamente como o mecanismo de `public/` do Vite é desenhado pra funcionar (arquivos que não devem ser processados pelo bundler). O único arquivo que o Vite de fato bundla é um `index.html` novo, mínimo, criado na raiz do projeto — 12 linhas, só um `<meta http-equiv="refresh">` redirecionando pra `/ilha_aprendiz.html`. Ele existe só porque Rollup exige pelo menos 1 entry HTML pra `vite build` não falhar, e porque dá uma URL "/" sensata em dev/preview/hospedagem estática.

**Alternativas descartadas:**
- **`root: 'app'` direto, com `app/ilha_aprendiz.html` como entry do Rollup:** parecia mais direto, mas como Vite trata QUALQUER `<script src="...">`/`<link href="...">` relativo referenciado por um HTML que É entry do Rollup como candidato a asset processado (copiado com hash de conteúdo no nome, potencialmente minificado), isso arriscava silenciosamente renomear/minificar os 24 scripts clássicos — quebrando os 21+ `onclick="nomeDaFuncao()"` inline do HTML, que dependem do nome exato da função global permanecer intocado. Descartado por risco real, não hipotético: o comportamento exato de minificação de script clássico (não-módulo) referenciado em HTML entry não é 100% documentado/garantido a preservar nomes de topo em toda versão do Rollup/esbuild, e o custo de um erro aqui (progresso pedagógico quebrado silenciosamente em produção) é alto demais pra apostar nisso sem necessidade.
- **Converter os scripts pra ES Modules:** explicitamente proibido pelo escopo desta fase.

**O que isso significa na prática:** o app real nunca passa pelo pipeline de bundle do Vite nesta fase — zero risco de minificação quebrar `onclick=`, zero risco de hash de nome de arquivo quebrar `MEDIA_BASE`/paths de mídia. `dist/ilha_aprendiz.html` é byte-idêntico a `app/ilha_aprendiz.html` (confirmado por diff no build de verificação), e todo asset copiado preserva nome e conteúdo exatos. O preço pago: nenhum dos benefícios de bundling/tree-shaking/HMR real do Vite se aplica ao código do jogo nesta fase — só ao stub de redirecionamento. Isso é uma trade-off deliberada e documentada, não uma limitação descoberta depois: exatamente o "menor delta possível" pedido. Modularização real (ES Modules, bundling de verdade) fica pra uma fase futura, só depois que a arquitetura de globais compartilhados (ver `docs/GLOBALS_INVENTORY.md`) for revisitada de propósito.

**`base: './'`** (path relativo) escolhido em vez de assumir `/` ou um subpath fixo, porque o GitHub Pages atual (`https://juliosud17.github.io/Projeto-Filhos/ilha-aprendiz/app/ilha_aprendiz.html`) serve os arquivos-fonte direto, sem usar `dist/` nenhum — este `base` não afeta esse deploy hoje. Quando (e se) `dist/` vier a ser publicado de verdade, `base` pode precisar de ajuste dependendo de onde for hospedado; documentado como pendência explícita em `docs/VITE_MIGRATION_CHECKLIST.md`.

**Testado:** build de verificação (`npm run build`) produziu `dist/ilha_aprendiz.html` byte-idêntico ao original (diff vazio), os 24 `<script src>` na mesma ordem e sem `type="module"`, CSS idêntico, asset de amostra (mapa da Ilha das Letras) copiado com o mesmo hash MD5. Smoke test funcional com Chromium headless contra `vite preview` e `vite dev`: tela inicial carrega, seleção de criança navega corretamente, todos os globais esperados (`showScreen`, `selectChild`, `AudioManager`, `startGame` etc.) definidos, zero erro de página (só um 404 de `favicon.ico`, pré-existente e não relacionado). Persistência testada de ponta a ponta: `activityLevel`/`state.totalStars` gravados via `saveProgress()` sobrevivem a um reload da página sob `http://localhost:4173`. Suíte completa (`npm test`): 38/39 arquivos sem falha (mesma falha conhecida de sempre em `qa_test_regression.js`), incluindo o novo `testes/qa_test_vite_build.js` (10/10), que valida a saída do build sem depender de jsdom.

**Atualização (mesmo dia):** confirmado o ambiente real do Júlio (Node v24.14.1, npm 11.11.0), `vite` atualizado de `^5.4.21` pra `^8.2.2` (última estável da linha 8.2.x, compatível com a exigência de engine do Vite 8). `vite.config.js` renomeado pra `vite.config.mjs` só pra resolver um aviso de compatibilidade futura do Vite 8 sobre carregar config ESM como CommonJS — nenhuma linha de configuração mudou (`publicDir: 'app'` e `base: './'` permanecem idênticos). Toda a validação acima foi refeita do zero com Vite 8.2.2 e deu o mesmo resultado byte a byte.

---

## 2026-08-21 — Fase 3: ambientes formais + GitHub Pages passa a publicar `dist/` via GitHub Actions (Opção B)

**Contexto:** Fase 3 pedia auditoria + planejamento de ambientes (`development`/`production`/`staging` se justificado), a partir da base Vite entregue na Fase 2. A auditoria (relatório entregue ao Júlio, não versionado neste repositório) identificou o ponto de decisão real: hoje o GitHub Pages serve `app/` cru direto do repositório (`https://juliosud17.github.io/Projeto-Filhos/ilha-aprendiz/app/ilha_aprendiz.html`), nunca passando pelo `dist/` que a Fase 2 introduziu — então qualquer variável `VITE_*`/`import.meta.env` futura nunca teria efeito em produção real enquanto isso não mudasse.

**Decisão (aprovada pelo Júlio):** Opção B — produção deixa de servir `app/` direto e passa a publicar o artefato `dist/` gerado pelo Vite. Explicitamente **descartada** a alternativa de configurar GitHub Pages "Deploy from a branch" apontando pra uma pasta `dist/` dentro de `master` (Pages só aceita `/(root)` ou `/docs` nesse modo — não serve pra publicar o output de um build que vive em `ilha-aprendiz/dist/`). Caminho escolhido, mínimo e oficial: `master` → GitHub Actions → `npm ci` → `npm test` → `npm run build` → `dist/` → `actions/upload-pages-artifact` → `actions/deploy-pages` → GitHub Pages.

**Definição formal dos ambientes** (detalhe completo em `docs/ENVIRONMENTS.md`, novo nesta fase):
- `development` = `npm run dev` (Vite dev server, `localhost:5173`).
- `production` = `npm run build` → `dist/` → publicado no GitHub Pages via o workflow novo.
- `preview` (`npm run preview`) = validação local do artefato de produção, **não** um terceiro ambiente formal.
- `file://` = mantido funcionando por compatibilidade com o uso real diário, mas classificado como **modo legado/local**, não como `development` oficial.

**Staging:** não criado. Nenhuma infraestrutura hoje (deploy por PR, múltiplos domínios) justifica um terceiro ambiente; decisão revisitável só se essa necessidade surgir de verdade no futuro.

**`.env`/`.env.example`:** não criados. Continua não existindo nenhuma variável `VITE_*` real (confirmado de novo nesta fase — zero uso de `process.env`/`import.meta.env` em `app/`). Quando a primeira variável legítima surgir, `.env.example` nasce no mesmo commit, com o nome real — nada antecipado sem uso, mesma disciplina já registrada em `docs/SECRETS_AUDIT_FASE1.md`.

**`?calibrar=1`:** não migrado para `import.meta.env.DEV` nesta fase — continua funcionando como está, sem bug a corrigir; registrado só como melhoria possível para uma fase futura.

**Escopo explicitamente fora desta fase** (fica pra uma fase de CI/CD própria, não esta): pipeline complexo, múltiplos ambientes de deploy, preview por PR, matrix builds, release automation, versionamento automático/semantic release, deploy mobile, Docker, infraestrutura externa — além, claro, de Supabase/backend/auth/banco/PWA/Service Worker, já fora de escopo desde o início do plano mestre.

**O que muda de URL:** publicar `dist/` como artefato do Pages faz o conteúdo publicado virar a raiz do site (não mais um subcaminho `ilha-aprendiz/app/` dentro do repositório) — a URL pública esperada passa de `.../Projeto-Filhos/ilha-aprendiz/app/ilha_aprendiz.html` para algo como `.../Projeto-Filhos/` (redirecionando pra `/ilha_aprendiz.html`, mesmo padrão já usado em `development`/`preview`). Qualquer link salvo com a URL antiga deixa de funcionar quando a migração for aplicada de fato.

**Implementação desta fase:** workflow criado em `.github/workflows/deploy-pages.yml` (Node 22, `npm ci` reprodutível a partir do lockfile, `npm test` como gate antes do build, `npm run build`, `upload-pages-artifact`/`deploy-pages` com só as permissões necessárias `contents: read`/`pages: write`/`id-token: write`). **Nenhum commit, nenhum push e nenhuma alteração de configuração real do GitHub Pages foram feitos nesta etapa** — o workflow existe só localmente, aguardando revisão e autorização explícita do Júlio antes de qualquer commit/push, e a configuração de Pages (trocar "Deploy from a branch" por "GitHub Actions" em Settings) continua pendente de autorização separada, só depois que o workflow em si for aprovado. Validação local (`npm test`, `npm run build`, `npm run preview`, QA manual) registrada no relatório de implementação entregue ao Júlio junto com esta entrada.

---

## 2026-08-21 — Fase 3 (revisão): `qa_test_regression.js` corrigido — não era flakiness aleatória, era o teste não esperar o `setTimeout` real de `nextRound()`

**Contexto:** revisão da implementação da Fase 3 exigiu determinar objetivamente o exit code de `npm test` em ambiente limpo antes de qualquer gate de CI, com proibição explícita de `continue-on-error`/`|| true`/qualquer supressão genérica de falha. `npm test` (`testes/_run_all.js`) termina com `process.exit(1)` sempre que qualquer arquivo da suíte falha — então a falha "conhecida" de `qa_test_regression.js`, documentada desde a Fase 1 como "flakiness de `setTimeout`/jsdom", faz `npm test` retornar código 1, o que bloquearia o `build`/`deploy` do workflow novo desta fase.

**Diagnóstico:** rodei `qa_test_regression.js` isoladamente 5 vezes seguidas, em ambiente limpo — **falha 100% das vezes, de forma idêntica** (`RESULT: 11 passed, 1 failed`, sempre a mesma checagem: "session ended on end screen"). Isso **não é flakiness aleatória** (resultado variável entre execuções) — é uma falha determinística. Causa raiz encontrada por leitura direta do código: a seção do teste que simula "ganhar todas as rodadas" de `pares_minimos` clica sincronamente nos botões de cada rodada, num loop `for`, sem nenhuma espera entre uma rodada e outra. Mas `nextRound()` (`js/game-loop.js`) — responsável tanto por renderizar a rodada seguinte quanto por detectar fim de sessão (`state.round > state.totalRounds` → `endSession()`) — só é chamado depois de um `setTimeout(nextRound, opts.nextRoundDelay || 1100)` real (`registerAnswer`, `js/game-loop.js` linha 306). Como o script de teste inteiro roda de forma síncrona dentro do mesmo `<script>` que o jsdom executa, o `for` do teste tenta clicar nos botões da rodada seguinte **antes** desse timer de ~1,1s disparar — encontra os botões (já desabilitados) da rodada anterior, nenhum clique aumenta `state.sessionStars`, e a partir da rodada 1 a "vitória simulada" nunca acontece de verdade — por isso a sessão nunca chega em `screen-end` de fato.

**Por que isso é problema exclusivo do teste, não de produção:** o comportamento real do app (rodada avança de verdade depois de ~1,1s, sessão termina na tela de fim depois da última rodada) nunca foi violado — é exatamente o comportamento pretendido desde que `nextRoundDelay` foi introduzido. O harness de teste é que fazia uma suposição errada (que a transição de rodada é síncrona), incompatível com um `setTimeout` real. Confirmado que esse desalinhamento é o único problema: nenhuma outra checagem do arquivo (as 11 anteriores, sobre menu/módulos/painel/desbloqueio) falha nunca.

**Correção aplicada — só no arquivo de teste, nenhuma linha de `app/js/`/`app/data/` tocada:** a seção de simulação de rodadas de `qa_test_regression.js` foi envolvida numa função assíncrona (`async function(){...})()`) que efetivamente `await`a um `setTimeout` real (1200ms, com folga sobre os 1100ms de produção) entre um clique e a busca pelos botões da rodada seguinte, e mais uma espera final antes de checar `screen-end` — dando tempo real para o `setTimeout(nextRound, ...)` de produção disparar, exatamente como um navegador de verdade faria. Nenhum valor de produção (`nextRoundDelay`, lógica de `registerAnswer`/`nextRound`) foi alterado.
