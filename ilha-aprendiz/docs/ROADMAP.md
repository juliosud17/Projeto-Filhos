...
4. ~~Fase 2 — Migração controlada para Vite/HTTP~~ — **feita** em 2026-08-21. `npm run dev`/`npm run build`/`npm run preview` funcionando, servindo o mesmo `app/ilha_aprendiz.html` e os mesmos 24 scripts clássicos sem nenhuma conversão pra ES Modules — Vite usado só como infraestrutura (`publicDir` passthrough), zero reescrita de arquitetura. `file://` e GitHub Pages (que servia os arquivos-fonte direto, sem `dist/`) continuaram funcionando sem alteração **até a Fase 3** — ver item 5 abaixo, que mudou esse comportamento. Detalhe completo em `docs/DECISOES.md` (entrada de 2026-08-21) e `docs/DEV_SETUP.md`.
5. ~~Fase 3 — Ambientes~~ — **feita** em 2026-08-24, publicada em `e2b4b46`. Ambientes formais definidos (`development`/`production`/`preview`, sem staging); GitHub Pages passa a publicar o artefato `dist/` via GitHub Actions (`deploy-pages.yml`), não mais os arquivos-fonte direto; `file://` reclassificado como modo legado/local de compatibilidade, não `development` oficial. Detalhe completo em `docs/ENVIRONMENTS.md` e `docs/DECISOES.md` (entrada de 2026-08-21).
6. ~~Fase 4 — Backend/Banco (Supabase)~~ — **feita** em 2026-08-25. Infraestrutura Supabase reproduzível (CLI, ambientes, SDK, cliente frontend isolado, workflow local `db:*`, ciclo de migration documentado) sem acessar dados de usuário e sem alterar o comportamento atual do app — `storage.js`/`localStorage` intocados, nenhum Auth, nenhuma tabela de domínio, nenhuma migration de domínio. Detalhe completo em `docs/SUPABASE.md` e `docs/DECISOES.md` (entrada de 2026-08-24).
7. Fase 5 — próxima fase, ainda não iniciada. Escopo não detalhado neste roadmap (ver `docs/DECISOES.md` para os limites já registrados nas Fases 4.1–4.4).
