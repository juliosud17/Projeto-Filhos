...
## Ilha das Letras — mapa interativo de Português (2026-08-16, UX refinada em 2 rodadas em 2026-08-17)
...
- **Linguagem de aventura na tela de Atividades:** `renderAtividades()` (`js/navigation.js`) mostra o nome/ícone da região (ex. "🌳 Floresta do Alfabeto") em vez do nome curricular ("Módulo 1 · Alfabeto e Sílabas") quando o módulo aberto tem uma entrada correspondente em `PT_MAPA_REGIOES` — ou seja, só quando é Português; subtítulo também contextual desde a rodada 3 ("Escolha seu próximo desafio" sem progresso, "Continue sua aventura — X de Y" com progresso). O dado curricular original em `registro-modulos.js` não muda (painel adulto/admin continuam mostrando o nome oficial); é puramente uma bifurcação de apresentação. Matemática não tem entrada nesse array, comportamento idêntico ao de sempre.
