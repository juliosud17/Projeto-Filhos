...
- **Coordenadas dos hotspots** (`data/mapa-portugues.js`) foram recalibradas visualmente contra o asset real (ver abaixo) — ainda estimativa a olho, não medição exata de pixel; ajuste fino com `?calibrar=1` na URL (clique no mapa imprime a coordenada no console).
