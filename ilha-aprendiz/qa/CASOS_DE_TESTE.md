...
| `qa_test_mapa_portugues.js` | Ilha das Letras (`js/mapa-portugues.js`): 8 hotspots acessíveis, os 5 estados visuais (marcador compacto + anel de progresso), popover sob demanda por estado (conteúdo, CTA presente/ausente, só 1 aberto por vez, **fechado por padrão** ao entrar), clique no marcador abre popover (não navega), CTA navega (Atividades/Projeto Leitor), "próximo destino" (`computeDestinoAtual`) em 3 cenários de progresso e confirmação de que não altera mastery/prova, 3 mensagens de cabeçalho contextuais (`mensagemDestinoAtual`), subtítulo contextual da tela de Atividades, cadeia completa de "Voltar" (exercício→Atividades→Mapa→Matérias), `calcularScrollCentralizado()` como função pura, regra `prefers-reduced-motion` presente no CSS, nome amigável de região em `atividades-title` com dado curricular original preservado, `backToModulos()` ciente da trilha, Matemática inalterada |
