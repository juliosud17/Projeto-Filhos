...
| `qa_test_mapa_portugues.js` | Ilha das Letras (`js/mapa-portugues.js`): 8 hotspots acessíveis, os 5 estados visuais (marcador compacto + anel de progresso), popover sob demanda por estado (conteúdo, CTA presente/ausente, só 1 aberto por vez), clique no marcador abre popover (não navega), CTA navega (Atividades/Projeto Leitor), "próximo destino" (`computeDestinoAtual`) em 3 cenários de progresso, nome amigável de região em `atividades-title` com dado curricular original preservado, `backToModulos()` ciente da trilha, Matemática inalterada |
